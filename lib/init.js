	...
	createFile(){
		...
		if(!fs.existsSync(preloadRule)){
			...
			if(pagesData['preloadRule']){
				jsonData["preloadRule"]=pagesData['preloadRule'];
			}
			fs.writeFileSync(preloadRule,JSON.stringify(jsonData));
		...
		if(pagesData['subPackages']){
			pagesData['subPackages'].forEach(_package=>{
				...
				if(!fs.existsSync(targetPath)){
					this.copyDirectory(_packagePath,subPackAgesDir);
				}
			});
			
			let subPackAgesDirItems = fs.readdirSync(subPackAgesDir);
			subPackAgesDirItems.forEach(item => {
				let dirPath=path.join(subPackAgesDirItems, item);
				if (fs.statSync(dirPath).isDirectory()) {
					...
					if(!fs.existsSync(itemRouterFilePath)){
						...
						pagesData['subPackages'].forEach(_package=>{
							_package.root='sub-packages/'+_package.root;
							if(path.join(path.resolve(), _package.root).indexOf(dirPath)>-1){
								jsonData['pages'].push(_package);
							}
						});
						fs.writeFileSync(itemRouterFilePath,JSON.stringify(jsonData));
						console.log('创建文件',itemRouterFilePath);
